...
人数制限ごとの「＋ 仮音声通話（4人）」などの作成用ボイスに入ると部屋ができ、全員が退出するとボイスとテキストの両方が削除されます。1人1部屋です。作成用ボイスの名前は、部屋のデフォルト名と連動します。
...
4. `/setup name` … 部屋のデフォルト名（省略可。初期値は `仮音声通話`）

カテゴリの権限で、ボットに「チャンネルを見る」「チャンネルの管理」「権限の管理」を許可してください。

## 日常の使い方

- `＋ 仮音声通話（4人）` などに入る → `仮音声通話_n` が作られ、自動で移動する
- 同じ名前のテキストも同時に作られる（カテゴリを見られるロールなら閲覧・投稿できる）
- `/setup name` でデフォルト名を変えると、これから作る部屋と作成用ボイスの名前が同じ名称になる
...
| `/setup name` | 新しく作る部屋のデフォルト名を変える（作成用ボイス名も連動） |
| `/setup debug` | ボットから見た権限を表示 |
| `/room create` | コマンドから部屋を作る（1人1部屋） |
| `/room rename` | 自分が作った部屋の名前を変える（ボイスとテキスト同時） |
